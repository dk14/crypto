...
> Concretely - in one of the html notebooks here (I wrote by hand) - you can notice that naive billinear map (DLP problem transfer from Bitcoin to DH) that sign of encrypted number can be guessed sometimes, since a bit is flipping faster when you progress with decimals. There are trivial statistical dependencies, even on the strongest bitcoin curve.
>
