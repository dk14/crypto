...
- develop algorithm (for firmware) for generating new seed, based on reported seeds
> reported seeds (generated by the tool) will become "would be a highly secure number if wasn't reported", it's proof-of-work. It makes sense, so far, to derive new private secrets by "adding more energy on top" (together with other sources of entropy, since such seed is public already). It would, however, require stronger metrics than bitmatch (so it has to be a paid feature).
> 
> Akin to PoW, p2p protocol can assign a reward (from user pool, based on score) to the reporter - **can 'sell' entropy**.
