...
> **And** for EU / UA / PL : [ZEN.com płatność QR dla UE / Polska](./docs/zen-qr.png). *Significant ZEN / SEPA donations (more than 1% of fund at the time of donation), can be converted to profit share manually (minus ZEN transaction / currency fee), **if** [contacted](mailto:sepa@doomsdayexplorer.online) with verifyable receipt, amount and btc reward address. Amounts must contain random numbers. Amounts (participating in profit-share) and reward addresses (together with funding tx created on your behalf, linking to re-direct address) will be publicly reported in `sepa.md`(the rest is your anonymity preference). Bud'mo!*
