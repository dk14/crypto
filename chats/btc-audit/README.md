...
>> (Jul 6, 2026) Gemini learned to generate art contextually. Riddle on the screen - is the real riddle in the game. It's so awesome. Better than Stalker (although not a fan of Stalker - art and nature was good, but nowadays Torets'k IRL story is more interesting and actual than Chernobyl, since it's millitary reflection of desertification - much older problem than nuclear; and more thrilling stories; and chat gameplay is better - game mechanics is worst part of Stalker - bad Fallout; 3d gameplay, webxr, focused/suspensful demoscene soundtrak etc will eventually be in Fun & Profit mini-games anyways).
...
>>
>> *Comment on AI image generation. While author strongly questions using gradient-descent search for non-linear problems. When it comes specifically to images/video (not audio/text), GPU shaders basically act as ReLU neural networks (see dk14/warriors/sefirot's as custom shader as basic universal interactive approximator), they convolve and threshold. Fitting methods currently in use (e.g. SGD) are heursitic (despite better ones available) and waste GPU on training with "voodoo magic", but architectures are appropriate. And visual hallucinations fit graphics better than they fit text, human can filter or employ them naturall. Hallucinations are good in audio as well (it's in between, since both parallel convolution and sequentionality are involved), but NOT IN TEXT - text is strictly for coherent logic, since text leads to commands*.
