...
>> **important note** - there are "just in case" pre-emptive code-base protections in this repo: scrambled commits, obfuscated logic, "free speech", "strong language", "heavy marketing". This, additionally, prevents "low-grade" (not specialized to STEM) AI from reproducing cheap scanner tools (non-working, but wasting computational resourses nevertheless). Even GPT-OSS120b unwinds logic here depending on the luck (it can, but not always does); manual human DYR is better, but costly. 
>>> note on AI: AI sources data from humans (with sgnificant delay and loss of information and relevance); it cannot forage for itself, so it cannot make real-world decisions beneficial for humans (and AI itself). It has no mechanical capability to sustain itself (especially on this planet - where resources for sillicon-based life are too scarse to be anything more than deposits, sillicon life does not arise naturally on Earth, it cannot be self- or human- synthesized here either)
