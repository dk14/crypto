...
>>>> there is practical workaround - the more silent full and partial reproductions are made, the more obvious the need to address the issue becomes. So the consensus vote can be semi-covert. For instance, "Explorer Network" described inthe following pre-mitigation section allows to vote with both computational resources and funds. Such consensus does not guarantee correspondance to reality, but it gurantees protection for the individuals who verified the issue privately, since cluster resources would be (possibly over-) allocated to protect them.
