...
Can also try ``` node enumerate.js urandom``` and ``` node enumerate.js clock```. The latter reproduces [libexplorer clock attack](https://thecyberexpress.com/bitcoin-keys-exposed-via-libbitcoin-explorer/). ``` enumerate ledger``` models thermal noise as superposition of sines and cosines, and enumerates deviations akin to libexplorer. The Ledger's secret factory model number (DUN) is modeled loosely, to avoid actual hacks.
