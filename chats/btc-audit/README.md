...
> Note for more complex metrics than bitmatch (advanced topic). We DO NOT invoke algorithmic complexity nonsense here. Machine (producing or tranforming sequence) would be fixed as inc dec jmpnz. And the metric is not length of a program, but amount of steps taken to tranform/produce sequence from initial state (minus steps taken on creating initial state, eg something known) - amount of energy spent to move from known to unknown. Abstract machine is most fundamental, lambdas and transform grammars are useful in [NLP](https://github.com/dk14/warriors/blob/main/eng.html) (as well as FP), but inc/dec/jmpnz machine is more fundamental and more meaningful (depsite all of them being complete), it operates directly on numbers.
