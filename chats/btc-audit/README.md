...
> Note for more complex metrics than bitmatch (advanced topic). We DO NOT invoke algorithmic complexity nonsense here. Machine (producing or tranforming sequence) would be fixed as inc dec jmpnz. And the metric is not length of a program, but amount of steps taken - amount of energy spent. Such machine is most fundamental, lambdas and transform grammars are useful in [NLP](https://github.com/dk14/warriors/blob/main/eng.html) (as well as FP), but abstract machine is more fundamental and more expressive (depsite all of them being complete).
