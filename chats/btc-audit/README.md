...
And for EU / Poland : [ZEN.com płatność QR dla UE / Polska](./docs/zen-qr.png). *Significant ZEN / SEPA donations (more than 1% of fund at the time of donation), can be converted to profit share manually, if [contacted](mailto:sepa@doomsdayexplorer.online) with verifyable receipt, amount and btc reward address. Amounts must contain random numbers. Amounts and reward address will be publicly reported in `sepa.md`(the rest is your preferense)*
