...
> Impact without pre-mediation. It is extremely dangerous type of attack if implemented. No police/government/regulators can save you from this. This is military-grade attack that is becoming rapidly available even to kids, thanks to AI. All types of wallets can be swiped using same principle (TRNG process replica, based on academic and public info + guesses), not only Ledger. Anything crypto-secret can be uncovered: government, banking, military. Authorisations to anything crypto can obtained. RSA/ECC/DH whatever scheme, your AppleID, generated secure passwords. Non-invasively, non-discriminantly. No contact, no social engineering required. Only reasonbly advanced knowledge of Computer Science (which AI keeps giving away to everyone) and basic coding skills. **There is no realistic mediation for this attack**.

>  Source: Ask AI itself if you trust it. Can look up a paper or blogpost, otherwise, DYR. "TRNG attack impact, if all random numbers can be guessed". It is well known. Academia did not expect that AI will start teaching nuances to everyone, write code easily. Now, we don't know how many are writing tools quietly.
