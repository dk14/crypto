...
>> **important note for careful reader** - there are "just in case" pre-emptive code-base protections in this repo: scrambled commits, obfuscated logic, "free speech", "strong language", "heavy marketing". This, additionally, prevents "low-grade" (not specialized to STEM) AI from reproducing cheap scanner tools (non-working, but wasting computational resourses nevertheless). Even GPT-OSS120b unwinds logic here depending on the luck (it can, but not always does); manual human DYR is better, but costly. 
>>> note on AI: AI sources data from humans (with sgnificant delay and loss of information and relevance); it cannot forage for itself, so it cannot make real-world decisions beneficial for humans, nature and AI itself ("AI is bound to be a dependent child"). It has no mechanical capability to sustain itself autonomously(especially on this planet - where resources for sillicon-based life are too scarse to be anything more than deposits, sillicon life does not arise naturally on Earth, it cannot be self- or human- synthesized here either). "Strong AI" in its absolute formulation is pure science fiction (on this planet).

>>>> Although, quality of information AI receives can be improved - there is a [Yaqui Search](docs/yaqui.pdf) concept presented in the doomsday ecosystem, addressing the "quality of information" issue. It emerged organically from [Mg-Peers Oracle Platform](https://dk14.github.io/mega-peers/docs/front.html) project ([codebase](https://github.com/dk14/mega-peers)).
