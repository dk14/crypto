...
> reported seeds (generated by the tool) will become "would be a highly secure number if wasn't reported", it's proof-of-work, high-entropy. It makes sense, so far, to derive new private secrets by "adding more energy on top" (together with other sources of entropy). It would, however, require stronger metrics than bitmatch (so it has to be a paid feature).
> 
