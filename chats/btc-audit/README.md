...
>  Source: Ask AI itself if you trust it. Can look up a paper or blogpost, otherwise, DYR. "TRNG attack impact, if all random numbers can be guessed". It is well known. I only made AI admit that not only human, but even (low-powered) hardware is a bad source of randomness, which is known too, semi-officially. What's new is that AI unwinds firmware's "security through obscurity" (the only defence it had, and it was psychological) easily, teaches you and writes code - this repo proves it. [Context](drama-context.md)
